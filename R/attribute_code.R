...
#' config <- add.resource.attribute.column(config,
...
#' @param label String. Label describing the Attribute.
#' @param description String. A explanation of what the Attribute represents.
#' @param from.existing String. URI of an existing attribute to reuse or extend. Can use the built-in list "Attibutes" to auto-fill.
#' @param definition.uri String. Source URI for the definition of the Attribute.
#' @param values Can  be:
#'    boolean: Whether to automatically generate attribute value resources for each unique value in the column.
#'    A "values" object. Created by the values() function filled with attribute.value(). The distinct attribute values which can be used in the column
...
#' data(example.data)
#' config <- create.config(example.data, title = "sweden_at_eurovision_no_missing")
#' config <- add.attribute.column(config,
#'   column.name = "Status", label = "status",
#'   values = values(attribute.value(label = "Final"), attribute.value(label = "Provisional"))
#' )
#'
#' @export
add.attribute.column <- add.resource.attribute.column
...
#' @param from.existing String. The URI to an existing attribute values to extend. Users can use the list AttibuteValues for auto-filling.
#'
...
attribute.value <- function(label, description = "", definition.uri = "", from.existing = "") {
  ...
  class(value) <- "value"
  value$label <- label
  value$description <- if (!is.empty.string(description)) description
  value$definition_uri <- if (!is.empty.string(definition.uri)) definition.uri
  value$from_existing <- if (!is.empty.string(from.existing)) from.existing
  return(value)
}
