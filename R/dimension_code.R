...
#' @param from.existing String. URI of an existing dimension to reuse or extend. Use built-in list dimension.from.existing to auto-fill.
...
#' @param from.template String. Dimension column templates. Use cuilt-in list dimension.from.template to autofill. Also, Users can use add.year.column() and other functions to achieve the same effect.
...
#' config <- add.dimension.column(config, column.name = "Year",
#'    from.template = dimension.from.template$year,
#'    from.existing = dimension.from.existing$timePeriod)
#' @export
